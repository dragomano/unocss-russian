...
Существует множество других техник для предоставления подсказок программам чтения с экрана; например, [пресет Wind3](./wind3) включает [sr-only](https://unocss.dev/interactive/?s=sr-only) для визуального скрытия элементов, сохраняя их доступными для скринридеров.
