...
- [Префлайты](/config/preflights) — определяют глобальный «сырой» CSS.
...
- [Процессоры](/config/processors) — преобразуют сгенерированный CSS перед его предоставлением.
...
- **Тип:** `CSSProcessor<Theme>[]`

Пользовательские процессоры, которые преобразуют сгенерированные слои CSS. Подробнее о порядке их выполнения см. в разделе [Процессоры](/config/processors).
