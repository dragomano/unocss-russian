...
::: tip
В `presetWind4` ключ был изменен на `breakpoint`.

[Документация по теме `presetWind4`](/presets/wind4#theme).
:::

Если вы хотите унаследовать контрольные точки исходной темы (`original`), вы можете использовать `extendTheme`:
